  ...
  return (
    <>
      ...
      <main className="max-w-[1600px] mx-auto pt-24 pb-8 px-6 md:px-12 lg:px-20 xl:px-32 min-h-screen">
        ...
        {/* Benefit Banner */}
        <div className="flex items-center justify-center bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950/20 dark:to-purple-950/20 rounded-lg py-3 px-4 mb-6 gap-2">
          <SparklesIcon className="h-5 w-5 text-primary" />
          <span className="text-sm text-gray-700 dark:text-gray-300 font-medium">Exchange unused games for credits and unlock premium titles & bundles!</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {/* Inventory Section */}
          <section className="bg-white dark:bg-card rounded-lg shadow p-6 flex flex-col">
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center min-h-[180px] gap-3 relative" style={{ zIndex: 20 }}>
                ...
                <span className="font-medium text-gray-500 dark:text-gray-400">No games in inventory.</span>
                <Link href="/bundles" className="mt-2 bg-blue-700 text-white px-4 py-2 rounded hover:bg-blue-800 transition text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 pointer-events-auto" style={{ zIndex: 30, position: 'relative' }}>
                  Browse Bundles
                </Link>
              </div>
            ) : (
              <div className="flex gap-2 mb-4 overflow-x-auto">
                {inventoryKeys.map((key) => (
                  <div key={key.id} className="flex flex-col items-center border rounded-lg p-3 min-w-[140px] bg-gradient-to-br from-blue-50 to-white dark:from-blue-950/20 dark:to-card shadow hover:shadow-lg transition group relative">
                    <img
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400 text-center mb-1">{key.packageName}</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400 text-center mb-1">{key.publisherName}</div>
                    <div className="text-xs text-green-700 dark:text-green-400 text-center mb-1 font-medium">
          ...
          <section className="bg-white dark:bg-card rounded-lg shadow p-6 flex flex-col">
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center min-h-[180px] gap-3">
                <img src="/images/hero-background.jpg" alt="No games" className="w-24 h-24 opacity-40 mb-2" />
                <span className="font-medium text-gray-500 dark:text-gray-400">No exchangeable games available.</span>
              </div>
            ) : (
              <div className="flex gap-2 mb-4 overflow-x-auto">
                {exchangeableKeys.map((key) => (
                  <div key={key.id} className="flex flex-col items-center border rounded-lg p-3 min-w-[140px] bg-gradient-to-br from-purple-50 to-white dark:from-purple-950/20 dark:to-card shadow hover:shadow-lg transition group relative">
                    <img
                      src={key.steamGameMetadata?.screenshotUrlsJson ? JSON.parse(key.steamGameMetadata.screenshotUrlsJson)[0] : "/images/hero-background.jpg"}
                      alt={key.productTitle}
                      className="w-24 h-32 object-cover rounded mb-2 group-hover:scale-105 transition"
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400 text-center mb-1">{key.packageName}</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400 text-center mb-1">{key.publisherName}</div>
                    <div className="text-xs text-gray-700 dark:text-gray-300 text-center mb-1">
                      Cost: <span className="text-red-600 dark:text-red-400 font-bold">{key.creditsRequired} Credits</span>
                    </div>
                    <button
