...
export function ProductGrid({
  ...
  // Get locked tiers - use canonical for logic to determine what's locked
  const selectedTierIndex = selectedTier
    ? baseTiersCanonical.findIndex((tier) => tier.id === selectedTier.id)
    : -1;
  ...
  const lockedProductsOrdered = lockedTiers.flatMap((tier) =>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
          ...
          if (isUnlocked) {
            ...
            return tierProducts.map((product) => (
              ...
              >
                ...
                <div className="p-5">
                  <h3 className="text-lg font-medium line-clamp-2 text-gray-900 dark:text-foreground">
                    {product.title}
                  </h3>
            ...
            return (
              <LockedTierCard
                ...
                tierIndex={index}
                totalGamesToUnlock={tierWithItems.totalItemsToUnlock}
              />
            );
          }
        })}
        ...
        {selectedProduct && (
          <ProductDetailModal
            ...
            baseTierDisplayOrder={displayOrder} // Pass display order configuration
          />
        )}
      </div>
    </Card>
