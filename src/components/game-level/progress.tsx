...
export function GameLevelProgress({ currentLevel, nextLevel, revealedKeys, totalKeys }: GameLevelProgressProps) {
  const progressPercentage = nextLevel
    ? ((revealedKeys - currentLevel.required) / (nextLevel.required - currentLevel.required)) * 100
    : 100;
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4">
          <div className="space-y-4">
            {[currentLevel, ...(nextLevel ? [nextLevel] : [])].map((level, index) => (
              <motion.div
                key={level.level}
                ...
                className={`rounded-lg border p-4 ${
                  level === currentLevel ? 'bg-primary/5 border-primary/20' : ''
                }`}
              >
                ...
                {level === currentLevel && nextLevel && (
                  <div className="mt-2">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {nextLevel.required - revealedKeys} more keys until {nextLevel.title}
                    </p>
                  </div>
                )}
              </motion.div>
